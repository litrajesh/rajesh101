...
from AUIFNet import Encoder_Medium
...
scheduler4 = torch.optim.lr_scheduler.MultiStepLR(
        optimizer4, [41, 81], gamma=0.1)
...
name=['Encoder_Base_Train','Encoder_Detail_Train','Encoder_Medium_Train','Decoder_Train']

for iteration in range(epochs):

    Encoder_Base_Train.train()
    Encoder_Detail_Train.train()
    Encoder_Medium_Train.train()
    Decoder_Train.train()
    ...
    for step in range(Iter_per_epoch):
        ...
        M_K,eta_M,theta_M=Encoder_Medium_Train(img_input)
...
Encoder_Medium_Train.eval()
Encoder_Medium_Train.cpu()
Decoder_Train.eval()
Decoder_Train.cpu()
...
for i in range(4):
    ...
    if i == 0:
        ...
        torch.save(Encoder_Medium_Train.state_dict(), save_model_path)    
    elif i == 3:
