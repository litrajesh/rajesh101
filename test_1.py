...
if Test_data_choose=='Test_data_TNO':
    test_data_path = r'.\Datasets\Test_data_TNO'
elif Test_data_choose=='Test_data_NIR_Country':
    test_data_path = r'.\Datasets\Test_data_NIR_Country'
elif Test_data_choose=='Test_data_FLIR':
    test_data_path = r'.\Datasets\Test_data_FLIR'    
...
for i in range(int(Test_Image_Number/2)):
    if Test_data_choose=='Test_data_TNO':
        Test_IR = Image.open(os.path.join(test_data_path, f'IR{i+1}.bmp'))  # infrared image
        Test_Vis = Image.open(os.path.join(test_data_path, f'VIS{i+1}.bmp'))  # visible image
    elif Test_data_choose=='Test_data_NIR_Country':
        Test_IR = Image.open(os.path.join(test_data_path, f'IR{i+1}.png'))  # infrared image
        Test_Vis = Image.open(os.path.join(test_data_path, f'VIS{i+1}.png'))  # visible image
    elif Test_data_choose=='Test_data_FLIR':
        Test_IR = Image.open(os.path.join(test_data_path, f'IR{i+1}.jpg'))  # infrared image
        Test_Vis = Image.open(os.path.join(test_data_path, f'VIS{i+1}.jpg'))  # visible image
    
    Fusion_image = Test_fusion(Test_IR, Test_Vis)
    
    # Convert fusion image to uint8 format
    ...
    os.makedirs(r'.\Test_result', exist_ok=True)
    Image.fromarray(Fusion_image_uint8).save(os.path.join(r'.\Test_result', f'F{i+1}.png'))